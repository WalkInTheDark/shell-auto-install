...
install_mysql() {
    ...
    test_info "
mysql安装完成，可以安装mysql-single启动单点
安装目录：${install_dir}/${mysql_dir}
日志目录：${log_dir}/${mysql_dir}
环境变量设置完毕，请退出当前终端后重新进入" "
mysql installation is complete, you can install mysql-single boot a single point
...
remove_mysql() {
    ...
    test_info "mysql卸载完成" "mysql Uninstall completed"
}
